...
const incomeHeads: { key: IncomeKey; icon: string; title: string; note: string; itr: string[] }[] = [
  ...
  { key: "other", icon: "＋", title: "Other Sources", note: "Interest, dividend, winnings and other income", itr: ["ITR-1", "ITR-2", "ITR-3", "ITR-4"] },
];
...
export default function Home() {
  ...
  const [winningsIncome, setWinningsIncome] = useState(0);
  ...
  useEffect(() => { const other = dividendIncome + interestIncome + depositInterest + otherIncome + winningsIncome; setValues(v => v.other === other ? v : { ...v, other }); }, [dividendIncome, interestIncome, depositInterest, otherIncome, winningsIncome]);
  ...
  const specialCapitalIncome = Math.max(0, Math.min(detailedSpecialIncome || fallbackSpecialIncome, Math.max(0, taxable - winningsIncome)));
  // Winnings taxable u/s 115BB / 115BBJ are not reduced by Chapter VI-A deductions.
  const specialWinningsIncome = Math.max(0, winningsIncome);
  const specialIncome = specialCapitalIncome + specialWinningsIncome;
  const normalIncome = Math.max(0, taxable - specialWinningsIncome - specialCapitalIncome);
  const basicExemption = basicExemptionLimit(regime, personal.dob);
  const specialBasicExemptionAdjustment = Math.min(specialCapitalIncome, Math.max(0, basicExemption - normalIncome));
  ...
  const specialCapitalRateTax = detailedSpecialIncome > 0 ? detailedSpecialRateTax : fallbackSpecialTaxableBase * fallbackSpecialRate;
  const winningsTax = specialWinningsIncome * .30;
  const specialRateTax = specialCapitalRateTax + winningsTax;
  ...
  const capitalSpecialTaxWorking = detailedSpecialIncome > 0 ? [
  ...
  ] : specialCapitalIncome > 0 ? [{ label: committedTaxData.capitalSection === "111A" ? "Short-Term Capital Gain u/s 111A" : committedTaxData.capitalSection === "112A" ? "Long-Term Capital Gain u/s 112A" : "Long-Term Capital Gain u/s 112", income: specialCapitalIncome, exemption: fallbackSpecialBeneficialAdjustment, threshold: committedTaxData.capitalSection === "112A" ? Math.min(125000, Math.max(0, specialCapitalIncome - fallbackSpecialBeneficialAdjustment)) : 0, rate: fallbackSpecialRate, tax: specialCapitalRateTax }] : [];
  const specialTaxWorking = [
    ...capitalSpecialTaxWorking,
    ...(specialWinningsIncome > 0 ? [{ label: "Winnings Income u/s 115BB / 115BBJ", income: specialWinningsIncome, exemption: 0, threshold: 0, rate: .30, tax: winningsTax }] : []),
  ];
    ...
    <main className="app-shell">
      ...
      <div className="workspace">
        ...
        {current ? <section className="entry-card">
          ...
          {active === "other" && <><div className="form-grid"><Field label="Dividend Income" value={dividendIncome} onChange={setDividendIncome} /><Field label="Interest from Savings Bank" value={interestIncome} onChange={setInterestIncome} /><Field label="Interest from Deposit" value={depositInterest} onChange={setDepositInterest} /><Field label="Winnings Income u/s 115BB / 115BBJ" value={winningsIncome} onChange={setWinningsIncome} hint="Lottery, crossword puzzles, races, card games or online-game winnings - taxable at 30%" /><Field label="Other Income" value={otherIncome} onChange={setOtherIncome} hint="Income other than dividend, interest and winnings" /></div><label className="description-field"><span>Details of Other Income</span><textarea value={otherDescription} onChange={(e) => setOtherDescription(e.target.value)} placeholder="Describe the nature and source of other income, for example family pension, gifts or miscellaneous receipts." /></label></>}
